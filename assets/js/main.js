...
function handelCalender() {
  ...
  const calender = $('#calender-fa');
  const pwt = $('.pwt-datepicker-input-element');
  if (!pwt.length) {
    calender.persianDatepicker({
      "inline": true,
      "toolbox": {
        "enabled": false
      },
...
function updateComponents() {
  ...
  window.state.data.map(function (value, index, array) {
    value.matches.map(function (match) {
      ...
      const matchStatus = $('#match-status-' + match.id);
      ...
      if (match.status.started && !match.status.cancelled && !match.status.finished) {
        matchStatus.text(match.status.liveTime.short);
      }
...
function getDate() {
  ...
  dateInfo['date'] = date;
  dateInfo['paramDate'] = date.format('YYYYMMDD');
...
function matchComponent(match) {
  ...
  let liveTime = !finished && started ? match.status.liveTime.short : finished ? 'Finished' : '';

  let html = '';

  html += `<tr id="match-${match.id}">
                <td class="text-right">${match.time}</td>
                <td class="text-left">${match.away.name}</td>`;

  if (started) {
    html += `<td class="bg-score" id="match-score-away-${match.id}"
        data-bs-target="${match.away.score}">${match.away.score}</td>
    <td class="bg-score" id="match-score-home-${match.id}"
        data-bs-target="${match.home.score}">${match.home.score}</td>`;
  } else if (finished) {
    html += `<td data-bs-target="${match.away.score}">${match.away.score}</td>
    <td data-bs-target="${match.home.score}">${match.home.score}</td>`;
  } else {
    html += `<td></td>
    <td></td>`;
  }
  ...
  html += `<td class="text-right">${match.home.name}</td>
                <td id="match-status-${match.id}">${liveTime}</td>
            </tr>`;
  return html;
}
